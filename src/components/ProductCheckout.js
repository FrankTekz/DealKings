...
export default function ProductCheckout(props){
    ...
    return(
            ...
            <div id="product-details-checkout">
                ...
                <p>${props.price}</p>
                <p>
                    <strong className="qty-btns" onClick={props.quantity == 1 ? () => null : () => decreaseQuantity(props.id)} >-</strong>
                    {props.quantity}
                    <strong className="qty-btns" onClick={() => increaseQuantity(props.id)} >+</strong>
                </p>
                <p id="subtotal">${props.price * props.quantity}</p>
                    
            </div>
                <div id="price-remove">                    
                    <p id="remove-cart" onClick={() => removeFromCart(props.id)}>X</p>
                </div> 
            </div>           
    )
